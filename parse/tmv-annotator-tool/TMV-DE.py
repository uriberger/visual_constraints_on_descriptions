...
# author: Anita Ramm
# Originally appeared in the tmv-annotator github package. Few adjustments made by Uri Berger (March 2022)
...
out_file_path = os.path.join(out_dir_name, sys.argv[2] + ".verbs")
out_file = codecs.open(out_file_path, "w", encoding='utf8')
sein_verb_file = codecs.open(sys.argv[3], "r", encoding='utf8')
...
def extractVerbalDepDict(fin_pos, dep_dict, pos_dict):
    verbal_tags = ["VAFIN", "VMFIN", "VVFIN", "VAINF", "VMINF", "VVINF", "VAPP", "VMPP", "VVPP", "PTKZU", "VAIZU",
                   "VMIZU", "VVIZU", "PTKNEG", "PTKVZ", "KON"]
    verbal_rels = []
                ...
                if (curr_pos in verbal_tags):
                    ...
                    if i not in fin_pos:
                        ...
                        if "INF" in curr_pos or "IZU" in curr_pos:
                            temp_res[str(
                                i) + "#" + curr_pos + "#" + curr_rel + "#" + curr_morph + "#" + curr_token + "#" + curr_lemma + "#" + str(
                                last_fin_pos)] = createDepDict(i, fin_pos, inf_vcs, coord)
...
def getPunctuationDeps(dep_dict, pos_dict):
    ...
    def searchPunct(curr_id, temp_dict, pos_dict):
        ...
        if curr_id in pos_dict:
            if pos_dict[curr_id].split("\t")[11] == "--" and (pos_dict[curr_id].split("\t")[5] in ["$.", "$,"] or (
                    pos_dict[curr_id].split("\t")[5] in ["$("] and not pos_dict[curr_id].split("\t")[
                1].lower() == "\"")):
                res.append(curr_id)
...
def getTenseDE(chain_dict, sein_verb_list):
    ...
    def deriveTMDE(pos_seq, tensed_verb, dep_verbs_dict):
        ...
        elif (len(pos_seq) == 2 or len(pos_seq) == 3) and not checkForParticiple(pos_seq) and checkForInfinitive(
                pos_seq):
